...
const libcairo_gobject_splitpath = ["bin", "libcairo-gobject-2.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libcairo_gobject = "libcairo-gobject-2.dll"


# Relative path to `libcairo_script_interpreter`
const libcairo_script_interpreter_splitpath = ["bin", "libcairo-script-interpreter-2.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libcairo_script_interpreter = "libcairo-script-interpreter-2.dll"
