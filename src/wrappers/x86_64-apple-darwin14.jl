...
const libcairo_gobject_splitpath = ["lib", "libcairo-gobject.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcairo_gobject = "@rpath/libcairo-gobject.2.dylib"


# Relative path to `libcairo_script_interpreter`
const libcairo_script_interpreter_splitpath = ["lib", "libcairo-script-interpreter.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libcairo_script_interpreter = "@rpath/libcairo-script-interpreter.2.dylib"
